...
export default function App() {
  ...
  const requestPermissions = async () => {
    if (Platform.OS === 'android') {
      try {
        console.log(
          'Solicitando permissões para gravação de áudio e armazenamento...'
        );
        ...
        if (
          ...
          console.warn(
            'Permissões foram negadas permanentemente (never_ask_again). Redirecionando para as configurações.'
          );
          Alert.alert(
  ...
  const startRecording = async () => {
    ...
    if (!permissionGranted) {
      setError(
        'Permissões necessárias para gravação de áudio não foram concedidas.'
      );
